...
const useSliderStore = defineStore("sliderStore", () => {
  ...
  const retrieveSlidersFromDB = async () => {
    try {
      ...
      sliders.value = response.data; // Assuming the API response contains the sliders array
  ...
  const deleteSlider = async (id) => {
    try {
      ...
      sliders.value = sliders.value.filter(slider => slider.id !== id); // Remove the deleted slider from the local state
  ...
  const createSlider = async (sliderData, headers = {}) => {
    try {
      ...
      await retrieveSlidersFromDB(); // Refresh the list of sliders after creating a new slider
  ...
  const updateSlider = async (id, sliderData, headers = {}) => {
    try {
      ...
      await retrieveSlidersFromDB(); // Refresh the list of sliders after updating the slider
